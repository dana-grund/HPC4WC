...
class Solver:
    ...
    * v			latitudinal fluid velocity
    ...
    def __init__(self, T, M, N, IC, CFL, diffusion):
        ...
        self.theta1D = np.linspace(-self.dtheta/2.0-math.pi, math.pi + self.dtheta/2.0, self.N+2)
        print(self.theta1D/math.pi*180.)
        ...
        assert(IC in range(3)), "Invalid problem ID. See code documentation for implemented initial conditions."
        self.IC = IC
    ...
    def solve(self, verbose, save):
        ...
        if (verbose > 0):
            ...
            print("check y-dir periodicity:",np.all(self.h[1:-1,0]==self.h[1:-1,-3]),np.all(self.h[1:-1,-1]==self.h[1:-1,2]))
        # Save
        if (save > 0):
            tsave = np.array([[0.0]])
        ...
        while (t < self.T):
            ...
            self.h[1:-1,:] = np.concatenate((hnew[:,-2:-1], hnew, hnew[:,1:2]), axis = 1)
            #self.h[:,0]  = self.h[:,1]
            #self.h[:,-1] = self.h[:,-2]

            self.u[:,1:-1] = np.concatenate((unew[-2:-1,:], unew, unew[1:2,:]), axis = 0)
            # --- TO-DO: change the boundary also to periodic ---
            self.u[1:-1,:] = np.concatenate((unew[:,-2:-1], unew, unew[:,1:2]), axis = 1)
            #self.u[:,0]  = self.u[:,1]
            #self.u[:,-1] = self.u[:,-2]

            self.v[:,1:-1] = np.concatenate((vnew[-2:-1,:], vnew, vnew[1:2,:]), axis = 0)
            # --- TO-DO: change the boundary also to periodic ---
            self.v[1:-1,:] = np.concatenate((vnew[:,-2:-1], vnew, vnew[:,1:2]), axis = 1)
            #self.v[:,0]  = self.v[:,1]
            #self.v[:,-1] = self.v[:,-2]
            ...
            if (verbose > 0 and (n % verbose == 0)):
                ...
                print("check y-dir periodicity:",np.all(self.h[1:-1,0]==self.h[1:-1,-3]),np.all(self.h[1:-1,-1]==self.h[1:-1,2]))
                print("check y-dir:",np.all(np.abs(self.u[1:-1,1]-self.u[1:-1,-2])<1e-9),self.u[10,1],self.u[10,-2])
                print("check x-dir periodicity:",np.all(self.h[0,1:-1]==self.h[-3,1:-1]),np.all(self.h[-1,1:-1]==self.h[2,1:-1]))
